...
error () {
   ...
   echo "** ERROR: $@ **"
   echo ""
   echo "Usage: ./scripts/release/add-artemis-release.sh <path.to/artemis> <previous-release-version> <new-release-version>"
   echo ""
   echo "Must be run from within an artemis-website checkout root."
   echo ""
   echo "Example:"
   echo "cd <path.to>/artemis-website"
   echo "./scripts/release/add-artemis-release.sh ../release-work/artemis 2.30.0 2.31.0"
   echo ""
   exit 64
}
